...
def add_log_file_handler(
    ...
    --------
    str
        The log file path used by the handler.
        Returns an empty string if handler setup fails.
    """
    ...
    try:
        ...
        logger.exception("Failed to add file handler")
        return ""
...
def init_logger(
    ...
    if file_level is not None and log_fp is not None:
...
def add_log_extra(logger: logging.Logger, extra: dict) -> logging.LoggerAdapter:
    """Return a LoggerAdapter that injects contextual information into log records.
