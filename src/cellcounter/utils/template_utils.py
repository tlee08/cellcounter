...
def render_template(tmpl_name: str, pkg_name: str, pkg_subdir: str, **kwargs) -> str:
...
def save_template(
    tmpl_name: str, pkg_name: str, pkg_subdir: str, dst_fp: Path | str, **kwargs
) -> None:
    """Renders template with the given arguments and saves it to the out_fp."""
    dst_fp = Path(dst_fp)
    # Rendering the template
    ...
    *,
    ...
    for template_fp in templates_ls:
        ...
        if not overwrite and dst_fp.exists():
            # Check if we should skip importing (i.e. overwrite False & file exists)
